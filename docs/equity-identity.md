...
- A listing missing its issuer LEI also re-verifies when FIRDS supplies a valid LEI, rather than treating incomplete identity as current for thirty days.
- If GLEIF explicitly has no ISIN mapping, confirm the live home-share FIRDS record's exact issuer LEI through GLEIF's single-record endpoint; never fall back after an ambiguous, conflicting or failed ISIN lookup.
- This fallback records FIRDS as the ISIN-to-LEI evidence and GLEIF as the legal-entity evidence separately; only the requested ISIN is linked, without inventing a GLEIF relationship or sibling securities.
- Recovered legal entities must remain ACTIVE with ISSUED or LAPSED registration; mismatches and invalid identifiers fail closed under the existing directory write guards.
